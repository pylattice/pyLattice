% By Joh Schoeneberg 2017, Adapted from 'ScriptTrackGeneral' in 'trackWithGapClosing/Kalman'

% Francois Aguet, November 2010

function trackSettings = loadTrackSettingsAguet(varargin)
...
disp("load track settings aguet")
disp(ip.Results.MaxGapLength)
disp(gapRadius)
disp(linkRadius)
disp(ip.Results.Radius)
...
costMatrices(1).parameters.useLocalDensity = 0;  % 1 if you want to expand the search radius of isolated features in the linking (initial tracking) step.
...
costMatrices(2).parameters.useLocalDensity = 0; %1 if you want to expand the search radius of isolated features in the gap closing and merging/splitting step.
...
costMatrices(2).parameters.maxAngleVV = 360; %maximum angle between the directions of motion of two tracks that allows linking them (and thus closing a gap). Think of it as the equivalent of a searchRadius but for angles.
