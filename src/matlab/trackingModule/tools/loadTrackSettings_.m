...
function trackSettings = loadTrackSettingsJoh(varargin)
...
%% the following (double %%) is from the u-track sriptTrackGeneral
...
parameters.brownScaling = [0.25 0.01]; % aguet has [0.5 0.01] here. power for scaling the Brownian search radius with time, before and after timeReachConfB (next parameter).
...
parameters.ampRatioLimit = []; % auge has [0 Inf] here. for merging and splitting. Minimum and maximum ratios between the intensity of a feature after merging/before splitting and the sum of the intensities of the 2 features that merge/split.

parameters.lenForClassify = 5; %minimum track segment length to classify it as linear or random.

parameters.useLocalDensity = 0; % auget has 1 here. 1 if you want to expand the search radius of isolated features in the gap closing and merging/splitting step.
